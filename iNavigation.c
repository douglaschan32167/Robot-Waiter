...
void irobotNavigationStatechart(
	...
	/**************************************/
        ...
        if (abs(netAngle - angleAtManeuverStart) >= 90 && dio_A2){
			angleAtManeuverStart = netAngle;
			previous = TURNR;
            state = FORWARD;
        }else if (abs(netAngle - angleAtManeuverStart) >= 90 && !dio_A2){
			angleAtManeuverStart = netAngle;
            state = LOCATE;
        }
        ...
        if (abs(netAngle - angleAtManeuverStart) >= 90 && dio_A1){
			angleAtManeuverStart = netAngle;
			previous = TURNL;
            state = FORWARD;
        }else if (abs(netAngle - angleAtManeuverStart) >= 90 && !dio_A1){
			angleAtManeuverStart = netAngle;
            state = LOCATE;
        }
        distanceAtManeuverStart = netDistance;
